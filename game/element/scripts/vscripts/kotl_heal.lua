function CreateWisp(event)
    ...
    local trees = GridNav:GetAllTreesAroundPoint(caster:GetAbsOrigin(), 5000, true)
...
function OnDestroyCaster(event)
    local friends = FindUnitsInRadius( event.caster:GetTeamNumber(), event.caster:GetOrigin(), nil, 99999, DOTA_UNIT_TARGET_TEAM_FRIENDLY, DOTA_UNIT_TARGET_ALL, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES + DOTA_UNIT_TARGET_FLAG_FOW_VISIBLE, FIND_CLOSEST, false )
    print(#friends)
    for i=1,#friends do
