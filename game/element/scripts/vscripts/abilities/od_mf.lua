function OnSpellStart(event)
    ...
    for i=1,#enemies do
        ...
        --ParticleManager:SetParticleControl( nFXIndex1, 0, point )
        --ParticleManager:SetParticleControl( nFXIndex1, 1, Vector( 200, 0, 0 ) )
        ...
        Timers:CreateTimer(1.0,function()
            ...
            Timers:CreateTimer(0.099,function()
                local enemies2 = FindUnitsInRadius( caster:GetTeamNumber(), point, nil, 200, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_FOW_VISIBLE, FIND_CLOSEST, false )
                if #enemies2 > 0 then
                    for y=1,#enemies2 do
                        local damageTable = {
                        victim = enemies2[y],
