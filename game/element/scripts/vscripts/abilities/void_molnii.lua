function OnIntervalThink(event)
    ...
    if caster ~= nil then
        if caster:GetHealth()/caster:GetMaxHealth() < 0.34 then
            ...
            local enemies2 = FindUnitsInRadius( caster:GetTeamNumber(), point, nil, 9999, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_FOW_VISIBLE, FIND_CLOSEST, false )
            if #enemies2 > 0 then
