...
function CastTether( event )
	...
	if #friends > 0 and friends[1]:FindModifierByName("modifier_tether_ally_datadriven") == nil then
...
function CheckDistance( event )
	...
	if ability.tether_ally and ability.tether_target then
	...
	-- Break tether
	if ability.tether_ally then ability.tether_ally:ForceKill(false) end
	if ability.tether_target then ability.tether_target:ForceKill(false) end
end
