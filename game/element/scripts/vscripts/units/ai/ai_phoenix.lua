function Spawn( entityKeyValues )
	...
	Refresher = thisEntity:FindItemInInventory( "item_refresher" )
...
function PhoenixThink()
	...
	if _G.hardmode and Refresher ~= nil and not novaAbility:IsFullyCastable() and Refresher:IsFullyCastable() then
		return castRefresher()
	end
...
function castRefresher()
	ExecuteOrderFromTable({
		UnitIndex = thisEntity:entindex(),
		OrderType = DOTA_UNIT_ORDER_CAST_NO_TARGET,
		AbilityIndex = Refresher:entindex(),
		Queue = false,
	})
	return 0
end
