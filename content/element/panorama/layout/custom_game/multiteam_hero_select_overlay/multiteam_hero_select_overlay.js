...
function Hero()
{
	...
	if ( !localPlayerInfo )
		return;
	localPlayerInfo.possible_hero_selection = "npc_dota_hero_snapfire";
	$.Msg(Game)
}

(function()
{
	// $.Msg($.GetContextPanel().GetParent().GetParent().GetParent().FindChildTraverse("HeroPickRightColumn"));
	// $.Msg($.GetContextPanel().GetParent().GetParent().GetParent().FindChildTraverse("HeroList").GetChild(0));
